...
export async function setupCli() {
  ...
  const commands: CommandWithAction<CommandArg> = {
    cleanup: {
      ...
      action: async args => {
        const dirsToCleanup = args?.cleanupDir ? args.cleanupDir.split(',') : cliOptions.cleanupDirs;
        if (!dirsToCleanup || dirsToCleanup.length === 0) {
          console.log('No directories specified for cleanup.');
          return;
        }
        // await cleanup(cliOptions.cleanupDirs);
        await cleanup(dirsToCleanup);
      }
    },
    'update-pkg': {
